...
import type { ForceMode, DarkMode } from '../../types'
import Configure from './Configure'

const FORCE_MODE_KEY = 'battery-hub-ui-mode'

export default function Settings({ onManagedBuildingsChange, forceMode, onForceModeChange, darkMode, onDarkModeChange }: { onManagedBuildingsChange: () => void; forceMode: ForceMode; onForceModeChange: (m: ForceMode) => void; darkMode: DarkMode; onDarkModeChange: (m: DarkMode) => void }) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <section className="mb-8">
        ...
        <div className="bg-card rounded-xl border border-border-light p-5 shadow-sm">
      ...
      <section className="mb-8">
        <h3 className="text-sm font-medium text-neutral-500 uppercase tracking-wider mb-3">Appearance</h3>
        <div className="bg-card rounded-xl border border-border-light p-5 shadow-sm">
          <div className="flex gap-2">
            {(['light', 'dark', 'system'] as DarkMode[]).map(m => (
              <button
                key={m}
                onClick={() => onDarkModeChange(m)}
                className={`px-4 py-2 rounded-lg text-sm transition-colors ${
                  darkMode === m
                    ? 'bg-ai-blue text-white'
                    : 'bg-surface-tertiary text-neutral-500 hover:bg-surface-tertiary/80'
      ...
      <section className="mb-8">
        ...
        {imports.length === 0 ? (
          ...
          <div className="space-y-2">
            {imports.map(imp => (
              <div key={imp.id} className="bg-card rounded-xl border border-border-light px-4 py-3 flex items-center justify-between shadow-sm">
                <div>
